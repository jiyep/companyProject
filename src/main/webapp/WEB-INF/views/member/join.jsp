	...
	<div class="wrap">
			...
			<div class="tel_wrap">
				...
				<div class="tel_input_box">
					<input class="tel_input" oninput="autoHyphen(this)" maxlength="13" name="memberTel">
				</div>
				<span class="final_tel_ck" style="display:none">전화번호를 입력해주세요.</span>
				<span class="final_tel_lan_ck" style="display:none">올바른 형식으로 입력해주세요.(ex. 010-0000-0000)</span>
			</div>
			
			<div class="email_wrap">
				<div class="email_name">이메일*</div> 
				<div class="email_input_box">
					<input class="email_input1" id="user_email" name="emailId"><span id="middle">@</span><input class="email_input2" type="text" id="email_address" name="emailAdd" list="user_email_address">
					...
					<input type="hidden" class="email_input" id="memberEmail" name="memberEmail" value="">
				</div>
...
<script>
	...
	$(document).ready(function(){
		...
		$(".join_button").click(function(){
			...
			/* 이메일 유효성검사 */
			if(mail == "" && emailckCheck==false){
				$('.final_email_ck').css('display','block');
				emailCheck = false;
			}else if(emailckCheck==false){
			...
			//이메일 중복 체크
			var memberEmail = $("#memberEmail").val();			// .id_input에 입력되는 값
			var data = {memberEmail : memberEmail}				// '컨트롤에 넘길 데이터 이름' : '데이터(.mail_input에 입력되는 값)'
			...
			/* 최종 유효성 검사 */
			if(idCheck&&idckCheck&&pwCheck&&pwckCheck&&nameCheck&&emailCheck&&emailckCheck&&telCheck&&telLanCheck&&routeCheck&&checkES){
	...
	function email() {
		...
		if(email != "" && address != "" && address != "직접입력") {
			$("#memberEmail").val(email+middle+address);
		}
	};
